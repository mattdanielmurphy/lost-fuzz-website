...
- **Bandcamp:** [Apple II](https://en.wikipedia.org/wiki/Apple_II) interface.
...
- **Automated Bandcamp Sync:** A custom script (`scripts/sync-bandcamp.mjs`) scrapes Bandcamp for new releases and updates the bandcamp data automatically via GitHub Actions.
...
- **Apple II (Bandcamp):** A classic green-screen environment. Use `CATALOG` to see releases, `RUN [N]` to open them, or discover the `APPLE` and `MATRIX` easter eggs.
