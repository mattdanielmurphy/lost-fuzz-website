	...
	const commands = {
		help: (args: string[], execute?: (cmd: string) => void) => (
			<div className='flex flex-wrap gap-x-4 gap-y-1 my-1'>
				<span>COMMANDS:</span>
				<button onClick={() => execute?.("CATALOG")} className='terminal-link'>
					CATALOG
				</button>
				<button onClick={() => execute?.("RUN 1")} className='terminal-link'>
					RUN [N]
				</button>
				...
				<button onClick={() => execute?.("EXIT")} className='terminal-link'>
					EXIT
				</button>
		...
		catalog: () => (
			<div className='grid grid-cols-1 gap-1 my-2'>
				{releases.map((r, i) => (
					<div key={i}>
						{i + 1}. {r.name.toUpperCase()} - {r.artist?.toUpperCase() || "LOST FUZZ"}
					</div>
				))}
			</div>
		),
	...
	return (
		<main className='min-h-screen bg-black p-4 sm:p-8 font-[family-name:var(--font-vt323)] text-[#33ff33] text-xl uppercase overflow-y-auto'>
			<div className='max-w-6xl mx-auto'>
				<Terminal commands={commands} welcomeMessage={welcomeMessage} inputPrefix=']' className='mb-12' forceUppercase={true} disableAutoScroll={true} />
                ...
                [ SKIP_TO_BOTTOM ]
						...
						<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8'>
							{releases.map((release, index) => (
								<Link key={index} href={release.link} target='_blank' className='group block border border-[#33ff33] p-4 hover:bg-[#33ff33] hover:text-black transition-all'>
									...
									<div className='space-y-1'>
										...
										{release.artist && <div className='text-sm opacity-80'>{release.artist}</div>}
										<div className='text-xs mt-2 border-t border-current pt-1'>FILE: {index + 1}.EXE</div>
									</div>
								</Link>
							))}
						</div>
