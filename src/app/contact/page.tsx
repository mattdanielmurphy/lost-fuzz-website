	...
	const shellCommands = {
		help: (args: string[], execute?: (cmd: string) => void) => (
			<div className='flex flex-wrap gap-x-2 gap-y-1'>
				<span>COMMANDS:</span>
				<button onClick={() => execute?.("AVAIL")} className='terminal-link'>
					AVAIL
				</button>
				<button onClick={() => execute?.("LIST")} className='terminal-link'>
					LIST
				</button>
				...
				<button onClick={() => execute?.("INFO")} className='terminal-link'>
					INFO
				</button>
				<button
					onClick={() => execute?.("SAY HELLO")}
				...
				<button onClick={() => execute?.("CLOSE")} className='terminal-link'>
					CLOSE
				</button>
			</div>
		),
		...
		say: (args: string[]) =>
			args.join(" ").toUpperCase() || "HELLO FROM THE AMIGA",
		bandcamp: () => {
			router.push("/bandcamp")
		},
		spotify: () => {
			window.open(
				"https://open.spotify.com/artist/3Re7kFD4ofekyvC7fxG4A1",
				"_blank",
			)
		},
		amusic: () => {
			window.open(
				"https://geo.music.apple.com/artist/lost-fuzz/1636398740",
				"_blank",
			)
		},
		close: () => {
		...
		<main className='min-h-screen bg-[#0055aa] p-4 sm:p-12 font-[family-name:var(--font-press-start-2p)] text-white text-[12px] sm:text-xs relative'>
				...
				<div className='bg-[#aaaaaa] border-b-2 border-black p-2 flex items-center justify-between text-black mb-1'>
					<div className='flex gap-2 flex-shrink-0'>
						<button
							...
							className='w-6 h-4 bg-white border border-black flex items-center justify-center hover:bg-black transition-colors'
						>
					...
					<div className='flex gap-2 flex-shrink-0'>
						<Link
							href='/'
							className='w-6 h-4 bg-white border border-black hover:bg-black'
						></Link>
					</div>
				</div>
				...
				<div className='bg-[#0055aa] p-4 sm:p-10 border border-black space-y-6'>
					<div className='flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6'>
						...
						<div className='space-y-2 text-center sm:text-left'>
							<h1 className='text-lg sm:text-2xl text-[#ff8800] underline uppercase'>
								COMMUNICATION LINK
							</h1>
							<p className='leading-relaxed'>
								INITIATING CONNECTION TO LOST FUZZ...
							</p>
						</div>
					</div>
					...
					{status === "success" ?
						<div className='bg-white text-black p-8 border-4 border-[#ff8800] space-y-4'>
							...
							<p>
								THANK YOU FOR YOUR COMMUNICATION. A CONFIRMATION EMAIL HAS BEEN
								SENT TO YOUR INBOX.
							</p>
							<Link
								href='/'
								className='bg-[#aaaaaa] border-2 border-black px-4 py-2 text-black hover:bg-white transition-colors flex items-center gap-2'
							>
			...
			{shellOpen && (
				...
				>
					<div className='bg-white border-b-2 border-black p-1 flex items-center justify-between text-black cursor-move'>
						<div className='text-[8px] font-bold uppercase px-2 pointer-events-none'>
							AmigaShell
						</div>
						<button
							onClick={() => setShellOpen(false)}
							className='px-2 text-xs hover:bg-black hover:text-white transition-colors'
						>
							X
						</button>
					</div>
					<div
						className='p-2 h-48 overflow-y-auto bg-black text-white font-mono text-[10px] leading-tight cursor-text'
			...
			<div className='mt-8 flex gap-4 opacity-50 justify-center'>
				<button
					...
					className='w-8 h-8 border-2 border-white flex items-center justify-center italic font-bold hover:bg-white hover:text-[#0055aa] transition-colors'
				>
					A
				</button>
				<div className='w-8 h-8 border-2 border-white flex items-center justify-center italic font-bold'>
					V
				</div>
			</div>
		</main>
